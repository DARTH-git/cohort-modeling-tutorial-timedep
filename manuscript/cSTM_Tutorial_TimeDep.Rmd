...
Then, the transition probability dependent on state residence from Sick to Sicker, `v_p_S1S2_tunnels`, based on a Weibull function in Equation \@ref(eq:tp-from-H-weibull) is: 
```{r}
# Weibull parameters
...
Accounting for the effectiveness of treatment B is done similarly to the simulation-time-dependent approach above. We first transform `v_p_S1S2_tunnels` to a vector of rates, `v_r_S1S2_tunnels`, and multiply it by the hazard ratio of treatment B, `hr_S1S2_trtB`. Then, we transform back to probabilities to produce `v_p_S1S2_tunnels_trtB`, a vector of transition probabilities that account for the duration of S1 state-residence under treatment B.
