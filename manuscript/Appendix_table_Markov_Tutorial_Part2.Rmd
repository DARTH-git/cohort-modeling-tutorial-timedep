...
| \(M_{ad}\)   | Simulation time-dependent cohort trace matrix  | `m_M_ad`     | matrix          | (`n_t` + 1) x `n_states`    | numeric     |
...
|              |                                  |              |                 |                        |             |
|              |  **Weibull parameters**    |              |                 |                   |             |  
| $\lambda$    | scale of the Weibull hazard function  | `p_S1S2_lambda`      | scalar          |                   | numeric     |
| $\gamma$     | shape  of the Weibull hazard function | `p_S1S2_lambda`      | scalar          |                   | numeric     |
|              | vector of transition probability to become Sicker when Sick when incorporating time dependency on state residence | `v_p_S1S2_tunnels`      | vector          |  1 x `n_states_tunnels`                | numeric     |
|              |                                  |              |                 |                        |             |
