...
To derive a transition probability, $\text{tp}(\tau)$, as a function of time from $H(\tau)$, we use the following equation[@Diaby2014]
\begin{equation}
p_{\left[S1_{\tau},S2, \tau\right]} = 1-\exp{(H(\tau-1) - H(\tau))}
(\#eq:tp-from-H)
\end{equation}

Substituting the Weibull cumulative hazard in Equation \@ref(eq:tp-from-H), the transition probability from S1 to S2 as a function of the time the cohort spends in S1, $p_{\left[S1_{\tau},S2, \tau\right]}$, is
$$
